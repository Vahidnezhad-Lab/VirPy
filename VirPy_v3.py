...
def main():
    ...
    def transcriptome_alignment():
        cmd = 'bwa mem ' + fq1 + ' ' + fq2 + ' ' + transcriptome + '/*.fasta' + ' -t ' + n_thread + ' > ' + out + 'trans.sam'
        print('Running ', cmd)
        os.system(cmd)
    ...
    def sam_to_bam():
        cmd = 'samtools view -Sb -h ' + out + '/trans.sam > ' + out + '_trans.bam'
        print('Running ', cmd)
        os.system(cmd)
    sam_to_bam()
    ...
    def sort():
        cmd = 'samtools sort -@ ' + n_thread + ' ' + out + '/trans.bam -o ' + out + '_trans_Coord_sorted.bam'
        print('Running ', cmd)
        os.system(cmd)
    sort()
    ...
    def addreadgroup():
        cmd = 'picard AddOrReplaceReadGroups ' + 'I=' + out + '/accepted_hits.bam ' + 'O=' + out + '_RG.bam' + ' RGID=4 RGLB=twist RGPL=illumina RGPU=unit1 RGSM=' + out
        print('Running ', cmd)
        os.system(cmd)

        cmd1 = 'picard AddOrReplaceReadGroups ' + 'I=' + out + 'trans_Coord_sorted.bam' + 'O=' + out + '_Trans_RG.bam' + 'RGID=4 RGLB=twist RGPL=illumina RGPU=unit1 RGSM=' + out
        print('Running ', cmd1)
        os.system(cmd1)
    addreadgroup()
    ...
    def markduplicate():
        cmd = 'picard MarkDuplicates ' + 'I=' + out + '_RG.bam' + 'O=' + out + 'MarkDup_RG.Bam' + 'M=' + out + 'marked_dup_metrics.tx'
        os.system(cmd)
        cmd1 = 'picard MarkDuplicates ' + 'I=' + out + '_Trans_RG.bam' + 'O=' + out + 'MarkDup_Trans_RG.bam ' + 'M=' + out + 'trans_marked_dup_metrics.tx'
        os.system(cmd1)
    markduplicate()
    ...
    def stringtie():
        cmd = 'stringtie -A -l ' + out + '/accepted_hits.bam -G ' + index_dir + '/*.gtf -o ' + out + '/stringtie/stringtie.gtf -p ' + n_thread
        print('Running ', cmd)
        os.system(cmd)
    stringtie()

    def Indexmarkedbam():
        cmd = 'picard BuildBamIndex' + 'I=' + out + 'MarkDup_RG.Bam'
        print('Running ', cmd)
        os.system(cmd)
        cmd = 'picard BuildBamIndex' + 'I=' + out + 'MarkDup_Trans_RG.bam '
        print('Running ', cmd)
        os.system(cmd)
    Indexmarkedbam()

    def splitNcigar():
        cmd = './gatk SplitNCigarReads -R ' + index_dir + '/*.fasta -I ' + out + 'MarkDup_RG.Bam -O ' + out + '_splitNcigar.Bam '
        print('Running ', cmd)
        os.system(cmd)
    splitNcigar()

    def variantCalling():
        cmd = 'samtools mpileup -f ' + transcriptome + '/*.fasta  -g ' + out + 'MarkDup_Trans_RG.bam ' + '|bcftools call -m > ' + out + '_trans.vcf'
        print('Running ', cmd)
        os.system(cmd)

        ##os.system(cmd11)
        cmd = './gatk HaplotypeCaller -R ' + index_dir + '/*.fasta -I ' + out + '_splitNcigar.Bam -O ' + '_gen.vcf'
        print('Running ', cmd)
        os.system(cmd)
    variantCalling()

    def vcfmerg():
        cmd = 'bcftools merg ' + out + '/*.vcf -Oz -o ' + out + '_Merged.vcf'
        print('Running ', cmd)
        os.system(cmd)
    vcfmerg()
    ...
    def ROH():
        cmd = 'vcftools --vcf ' + '' + out + '_Merged.vcf --plink --out' + out + 'plink'
        print('Running ', cmd)
        os.system(cmd)
        cmd = 'plink --file ' + out + 'plink' + ' –homozyg '
        print('Running ', cmd)
        os.system(cmd)
    ROH()
    ...
    def sort():
        cmd = 'samtools sort -@ ' + n_thread + ' ' + out + '/acceptedHits.Aligned.sortedByCoord.out.bam -o' + out + '/acceptedHits.Aligned.sorted.out.bam'
        print('Running ', cmd)
        os.system(cmd)
    sort()
    ...
    def index():
        cmd = 'samtools index ' + out + "/acceptedHits.Aligned.sortedByCoord.out.bam"
        print('Running ', cmd)
        os.system(cmd)
    ...
    def express():
        cmd = 'express ' + index_vir + '/viruses.fasta ' + out + '/acceptedHits.Aligned.sorted.out.bam -o ' + out + '/eXpress/'
        print('Running ', cmd)
        os.system(cmd)
...
def main():
    ...
    def stringtie():
        cmd = 'stringtie ' + out + '/acceptedHits.Aligned.sortedByCoord.out.bam -A ' + out + '/stringtie/viral_abundance.txt -o ' + out + '/stringtie/stringtie.gtf -p ' + n_thread
        print('Running ', cmd)
        os.system(cmd)
    ...
    def kallisto():
        cmd = 'kallisto quant -i ' + index_vir + '/viruses.idx -o ' + out + '/kallisto/ ' + out + '/accepted_hits.Unmapped.out.mate1 ' + out + '/accepted_hits.Unmapped.out.mate2'
        print('Running ', cmd)
        os.system(cmd)
    ...
    def getTopVirus():
        ...
        for v in virus:
            ...
            if os.path.isfile(index_vir + "/annotationFiles/" + i + ".gtf"):
                cmd = 'featurecounts -p -a ' + index_vir + '/annotationFiles/' + i + '.gtf ' + '-o ' + out + '/featureCounts/' + i + '_counts.txt ' + out + '/unmapped_aln_sorted.bam'
                os.system(cmd)
                vir.append(v)
            elif os.path.isfile(index_vir + "/annotationFiles/" + i + ".saf"):
                cmd = 'featurecounts -p -F SAF -a ' + index_vir + '/annotationFiles/' + i + '.saf ' + '-o ' + out + '/featureCounts/' + i + '_counts.txt ' + out + '/unmapped_aln_sorted.bam'
                os.system(cmd)
                vir.append(v)
            else:
    ...
    def call_variants():
        ...
        cmd = 'freebayes -f ' + index_vir + '/viruses.fasta -b ' + out + '/unmapped_aln_Coord_sorted.bam --min-coverage ' + min_coverage + ' > ' + out + '/VariantCalling/variants.vcf'
        print('Running ', cmd)
        os.system(cmd)
        cmd = 'java -jar snpEff/snpEff.jar viruses ' + out + '/VariantCalling/variants.vcf > ' + out + '/VariantCalling/variants.ann.vcf'
        print('Running ', cmd)
        os.system(cmd)
    ...
    def consensus():
        cmd = './gatk IndexFeatureFile ' + out + '/VariantCalling/variants.ann.vcf'
        cmd = './gatk FastaAlternateReferenceMaker -R ' + index_vir + '/viruses.fasta -O ' + out + '/VariantCalling/consensus.fasta -V ' + out + '/VariantCalling/variants.ann.vcf'
        print('Running ', cmd)
        os.system(cmd)
        print('Running ', cmd)
        os.system(cmd)
...
if __name__ == '__main__':
    main()
/acceptedHits.Aligned.sortedByCoord.out.bam -A ' + out + '/stringtie/viral_abundance.txt -o ' + out + '/stringtie/stringtie.gtf -p ' + n_thread
        print('Running ', cmd)
        os.system(cmd)
